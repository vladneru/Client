...
auto Client::Entry()->void {
	...
	/*std::map<std::string, std::string>::iterator it=options_with_cert.find("cert_path");
	std::string f1 = it->second;
	std::map<std::string, std::string>::iterator it2 = options_with_cert.find("key_path");
	std::string f2 = it2->second;
	curl_easy_setopt(curl_, CURLOPT_SSLCERT, (char *)f1.c_str());
	curl_easy_setopt(curl_, CURLOPT_SSLKEY, (char *)f2.c_str());
	curl_easy_setopt(curl_, CURLOPT_CAINFO, (char *)f1.c_str());*/
	...
	for (int i = 0; i < file_names_dir.size(); i++) {
		...
		std::ifstream file_stream(direct[i], std::ios::binary);
